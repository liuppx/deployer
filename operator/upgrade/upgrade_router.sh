...
if [[ -f "${target_dir}/scripts/health-check.sh" ]]; then
    ...
    for ((attempt = 1; attempt <= max_health_check_attempts; attempt++)); do
        if (( WAIT_SECONDS > 0 )); then
            log "wait ${WAIT_SECONDS}s before router health check attempt ${attempt}/${max_health_check_attempts}"
            sleep "$WAIT_SECONDS"
        fi
        ...
        log "router health check failed on attempt ${attempt}/${max_health_check_attempts} with exit code ${health_check_status}"
    done

    if [[ $health_check_status -ne 0 ]]; then
        log "ERROR! router health check failed after ${max_health_check_attempts} attempt(s)"
        exit "$health_check_status"
    fi

    log "router health check passed"
else
    log "WARN! skip router health check, script is missing: ${target_dir}/scripts/health-check.sh"
fi
